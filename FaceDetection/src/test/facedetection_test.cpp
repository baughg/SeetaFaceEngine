...
int main(int argc, char** argv) {
	...
	std::string detection_report_file = std::string(img_path);
	...
	if (num_face)
	{
		...
		detection_report_file.append(".face");
		FILE* res_file = NULL;
#ifdef _WIN32
		fopen_s(&res_file, detection_report_file.c_str(), "wb");
#else
		res_file = fopen(detection_report_file.c_str(), "wb");
#endif

		if (res_file)
		{
			fwrite(&num_face, sizeof(int), 1, res_file);
			...
			for (int32_t i = 0; i < num_face; i++) {
				...
				fwrite(&box, sizeof(box), 1, res_file);
			}
			fclose(res_file);
		}
	}
